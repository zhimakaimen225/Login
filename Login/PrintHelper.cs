...
namespace Gd
{
    ...
    public class PrintHelper
    {
        ...
        public PrintHelper()
        {
            ...
            _pageSetupDialog.PageSettings = new PageSettings();
        }
        /// <summary>
        ///     打印
        /// </summary>
        ...
        public void Print(DataTable dt, string title)
        {
            try
            {
                PrintDocument printDocument1 = CreatePrintDocument(dt, title); 
                //初始化打印对话框对象
        ...
        private PrintDocument CreatePrintDocument(DataTable dt, string title)
        {
            ...
            _printDocument = new PrintDocument { DefaultPageSettings = _pageSetupDialog.PageSettings };
            //_printDocument.DefaultPageSettings.Landscape = true; //设置打印横向还是纵向
            //PLeft = 30; //DataTablePrinter.DefaultPageSettings.Margins.Left;
            _printDocument.PrinterSettings.PrintToFile = false;
        ...
        private void PrintDocumentPrintPage(object sende, PrintPageEventArgs @event)
        {
            ...
            foreach (DataRow dr in _dataTable.Rows)
            {
                ...
                {
                    int colwidth_withSpace = Convert.ToInt32(@event.Graphics.MeasureString(dr[i].ToString().Trim(), TableFont).Width);
                    int colwidth = Convert.ToInt32(@event.Graphics.MeasureString(dr[i].ToString(), TableFont).Width);
                    colwidth = (colwidth_withSpace - colwidth) * 2 + colwidth;
            ...
            if (_printingPageNumber == 1)
            {
                for (int cols = 0; cols <= _dataTable.Columns.Count - 1; cols++)
                {
                    ...
                    columnText = _dataTable.Columns[cols].ColumnName;
            ...
            if (_printingPageNumber == 1)
            {
                ...
                var arr = _headText.Split(new[] { "|" }, StringSplitOptions.None);
                if (arr.Length >= 1)
                {
